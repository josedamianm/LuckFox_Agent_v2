...
int main(void) {
    ...
    static const int btn_gpios[9] = {57, 69, 65, 67, 55, 64, 68, 66, 54};
    static const char *btn_names[9] = {"A", "B", "X", "Y", "UP", "DOWN", "LEFT", "RIGHT", "CTRL"};
    int btn_fds[9];
    for (int i = 0; i < 9; i++) {
        char path[64];
    ...
    while (g_running) {
        for (int i = 0; i < 9; i++) {
            ...
            if (pressed && !prev[i]) {
                ...
                switch (i) {
                    case 0: set_screen_color(lv_color_hex(0xFF0000), "RED (A)");        break;
                    case 1: set_screen_color(lv_color_hex(0x0000FF), "BLUE (B)");       break;
                    case 2: set_screen_color(lv_color_hex(0xFFFF00), "YELLOW (X)");     break;
                    case 3: set_screen_color(lv_color_hex(0x00FF00), "GREEN (Y)");      break;
                    case 4: set_screen_color(lv_color_hex(0xFF8000), "ORANGE (UP)");    break;
                    case 5: set_screen_color(lv_color_hex(0x800080), "PURPLE (DOWN)");  break;
                    case 6: set_screen_color(lv_color_hex(0x00FFFF), "CYAN (LEFT)");    break;
                    case 7: set_screen_color(lv_color_hex(0xFFFFFF), "WHITE (RIGHT)");  break;
                    case 8: set_screen_color(lv_color_hex(0xFF00FF), "MAGENTA (CTRL)"); break;
                }
                lv_refr_now(NULL);
            }
            prev[i] = pressed;
